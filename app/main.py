...
COLOR_SECOND = "#DAA520"          # Дополнительный фон (золотистый)
COLOR_ACCENT = "#B8860B"          # Акцентирование внимания (темно-золотистый)
COLOR_DISCOUNT_ROW = "#F4A460"    # Скидка >12% (светло-коричневый)
COLOR_ZERO_STOCK = "#ADD8E6"      # Товара нет на складе (голубой)
...
class DataService:
    ...
    def auth(login: str, password: str):
        """Проверка логина/пароля, возвращает UserInfo или None"""
        conn = get_connection()
        try:
    ...
    def get_manufacturers():
        """Получение списка производителей для фильтрации"""
        conn = get_connection()
        try:
            cur = conn.cursor()
    ...
    @staticmethod
    def get_manufacturers_for_combo():
        conn = get_connection()
        try:
            cur = conn.cursor(dictionary=True)
    ...
    def ensure_supplier(conn, name: str):
        """Добавление поставщика если не существует"""
        cur = conn.cursor()
    ...
    def product_exists_in_orders(product_id: int) -> bool:
        """Проверка, есть ли товар в заказах (для запрета удаления)"""
        conn = get_connection()
        try:
            cur = conn.cursor()
    ...
    def parse_article_pairs(text: str):
        """Парсит строку 'артикул1, кол-во1, артикул2, кол-во2'"""
...
class LoginDialog(QDialog):
    def __init__(self):
        ...
        btn_login.setStyleSheet(f"background:{COLOR_ACCENT}; color:white;")
        btn_guest.setStyleSheet(f"background:{COLOR_ACCENT}; color:white;")
...
class ProductFormDialog(QDialog):
    def __init__(self, product_id=None):
        ...
        btn_save.setStyleSheet(f"background:{COLOR_ACCENT}; color:white;")
    ...
    def load_lookups(self):
        ...
        for x in DataService.get_manufacturers_for_combo():
...
class ProductsWindow(QMainWindow):
    def __init__(self, user_data: UserInfo):
        ...
        self.setWindowTitle("СтройМатериалы - Список товаров")
        ...
        if os.path.exists(ICON_PNG):
            ...
            logo.setPixmap(pix)
        title = QLabel("ООО «СтройМатериалы» — Список товаров")
        ...
        btn_logout.setStyleSheet(f"background:{COLOR_ACCENT}; color:white;")
        ...
        self.btn_add.setStyleSheet(f"background:{COLOR_ACCENT}; color:white;")
    ...
    def load_manufacturers(self):
        ...
        for x in DataService.get_manufacturers():
            self.manufacturer_combo.addItem(x)
    ...
    def apply_filters(self):
        ...
        if self.is_manager_or_admin():
            ...
            if manufacturer and manufacturer != "Все производители":
                data = [x for x in data if (x.get("manufacturer_name") or "").lower() == manufacturer.lower()]
    ...
    def fill_table(self, rows):
        ...
        for i, row in enumerate(rows):
            ...
            # Подсветка строки (скидка >12% -> #F4A460)
